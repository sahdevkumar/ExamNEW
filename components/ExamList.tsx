...
import { Plus, Calendar, Pencil, Trash2, AlertTriangle } from 'lucide-react';
...
export const ExamList: React.FC = () => {
  ...
  const handleEdit = (ex: Exam) => {
      setFormData({ name: ex.name, code: ex.code });
      setEditingId(ex.id);
      setShowModal(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.name && formData.code) {
      if (editingId) {
          db.exams.update({ ...formData, id: editingId });
      } else {
          db.exams.add(formData);
      }
      setExams(db.exams.getAll());
      setShowModal(false);
      setFormData({ name: '', code: '' });
            ...
            <tbody className="divide-y divide-gray-200">
              {exams.map((e) => (
                <tr key={e.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 font-mono text-xs">{e.code}</td>
              ...
              {exams.length === 0 && (
                <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-gray-400">No exams found.</td>
                </tr>
              )}
            </tbody>
        </table>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <h3 className="text-xl font-bold mb-4">{editingId ? 'Edit Exam' : 'Add New Exam'}</h3>
